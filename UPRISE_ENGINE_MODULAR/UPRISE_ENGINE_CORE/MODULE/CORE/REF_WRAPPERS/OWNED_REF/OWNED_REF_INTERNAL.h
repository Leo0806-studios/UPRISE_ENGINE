...
namespace UPRISE_ENGINE {
    ...
    public:
        ...
        OwnedRef(OwnedRef<Type>&& other)noexcept(RW_USE_CPP_EXCEPTIONS_ == false) {//NOSONAR
            ...
            default: {
                this->ControlBlock = std::exchange(other.ControlBlock, IntegerTypeToPointer<ControlBlock_Base>(1ULL));
                break;
            }
            }
        ...
        OwnedRef& operator=(OwnedRef<Type>&& other)noexcept(RW_USE_CPP_EXCEPTIONS_ == false) {//NOSONAR
            switch (reinterpret_cast<uintptr_t>(this->ControlBlock)) {
            case 0: {
        ...
        OwnedRef(OwnedRef<OtherType>&& other)noexcept(RW_USE_CPP_EXCEPTIONS_ == false) {//NOSONAR
        ...
        bool operator ==(const OwnedRef<Type>& other) const {
         ...
         operator bool() const noexcept {//NOSONAR
            return this->ControlBlock != nullptr && reinterpret_cast<uintptr_t>(this->ControlBlock) > 2;
        }
        template< typename... Args>
        static OwnedRef<Type> Create(Args&&... args) {
            OwnedControlBlock<Type>* controlBlock = new OwnedControlBlock<Type>(); //-V2511
            controlBlock->Object = new Type{ std::forward<Args>(args)... }; //-V2511
