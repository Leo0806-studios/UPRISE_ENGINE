...
namespace UPRISE_ENGINE {
    ...
    private:
        ...
        bool Generic_isValid() {
            return PointerToIntegerType(ControlBlock) > static_cast<uintptr_t>(2);
        }
    ...
    public:
        ...
        WeakRef(WeakRef<Type, DoNullCheck>&& other)noexcept(RW_USE_CPP_EXCEPTIONS_ == false) {//NOSONAR
            if constexpr (DoNullCheck) {
        ...
        WeakRef& operator=(WeakRef<Type, DoNullCheck>&& other) {//NOSONAR
            if constexpr (DoNullCheck) {
        ...
        template<
            ...
            typename = std::enable_if <
            !std::is_same_v<Type, OtherType>>>
            WeakRef(const WeakRef<OtherType, DoNullCheck>& other) {//NOSONAR
            if constexpr (DoNullCheck) {
        ...
        template<
            ...
            typename = std::enable_if <
            !std::is_same_v<Type, OtherType>>>
            WeakRef(WeakRef<OtherType, DoNullCheck>&& other)noexcept(RW_USE_CPP_EXCEPTIONS_ == false) {//NOSONAR
            if constexpr (DoNullCheck) {
