...
namespace UPRISE_ENGINE {
    ...
    public:
        ...
        static __inline Quaterion __vectorcall FromEulerAngles(DATATYPES::Vector3 vec) {
...
#pragma warning (disable:5045)
        ...
        __inline  DATATYPES::Vector3 __vectorcall ToRotationVector() const {
            ...
            DATATYPES::Vector3 result{ 0,0,0 };

            if (s < 0.0001F) {
                // If s is close to zero, return the axis as (1, 0, 0)
                result = DATATYPES::Vector3(1.0F, 0.0F, 0.0F) * angle;
            }
                ...
                float ny = yC() / length;
                float nz = zC() / length;
                result = DATATYPES::Vector3(nx / s, ny / s, nz / s) * angle;
            }
            return result;
        }
#pragma warning(pop)
        __inline 	DATATYPES::Vector3 __vectorcall operator*(DATATYPES::Vector3 vec) {//NOSONAR
            float num = x() * 2.0F;
            float num2 = y() * 2.0F;
            float num3 = z() * 2.0F;
            float num4 = x() * num;
