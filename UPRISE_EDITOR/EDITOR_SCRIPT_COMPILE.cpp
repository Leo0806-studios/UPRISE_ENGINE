#include "pch.h"
#include "HEADER/EDITOR_SCRIPT_COMPILE.h"
...
int EDITOR::Copmiler::Scn_Dict_recurse(std::vector<std::filesystem::path>* vecToAdd, std::filesystem::path path)
{
    static int retval;
...
int EDITOR::Copmiler::Parse_File( std::ifstream& file, std::filesystem::path& path)
{
...
std::vector<std::filesystem::path> EDITOR::Copmiler::Scan_Dict(std::filesystem::path path)
{
...
int EDITOR::Copmiler::Parse_Source_Files(std::vector<std::filesystem::path>& Files)
{
    for (auto& File : Files) {
        std::ifstream script;
        script.open(File);
...
int EDITOR::Copmiler::PrepareCompiling()
{
...
int EDITOR::Copmiler::Compile(bool full)
{
    return 0;
}

int EDITOR::Copmiler::Link()
{
    return 0;
}

int EDITOR::Copmiler::Find_Decorated_Names(std::filesystem::path File)
{
    return 0;
}

int EDITOR::Copmiler::Create_def_File()
{
    return 0;
}
