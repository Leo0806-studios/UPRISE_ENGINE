...
public:
	...
	__forceinline static  Mat4 Look_At(DATATYPES::TSPVector3 eye, DATATYPES::TSPVector3 center, DATATYPES::TSPVector3 up) {
		//TrPr(ctx,__func__)
		center = (center - eye).Normalized();
		center.pad = -(center.point(eye));
		up = (up ^ center).Normalized();
		up.pad = -(up.point(eye));
		TSPVector3 u((center ^ up));
		u.pad = -(u.point(eye));
		Mat4 result(1);
	...
	__forceinline static  glm::mat4 Look_At_GLM( DATATYPES::FASTVec3 eye,  DATATYPES::FASTVec3 center,  DATATYPES::FASTVec3 up) {
		//TrPr(ctx,__func__)
		center = (center - eye).Normalized();
		center.pad() = -(center.point(eye));
		up = (up ^ center).Normalized();
		up.pad() = -(up.point(eye));
		FASTVec3 u((center ^ up));
		u.pad() = -(u.point(eye));
		...
		__m256 One = _mm256_set_m128(_mm_unpacklo_ps(center.vals, row3),  _mm_unpacklo_ps(up.vals, u.vals));

		__m256 Two = _mm256_set_m128(_mm_unpackhi_ps(center.vals, row3), _mm_unpackhi_ps(up.vals, u.vals));
